...
export default class Level extends Phaser.Scene {
  ...
  create() 
  {
    ...
    this.createPlayer();
    this.slimeNum = this.createSlime();
    this.slimeNum = this.createSlime();
    this.slimeNum = this.createSlime();
    this.initPhysics(); 
    this.handleNewGameMsg();
  }
  ...
  convertToGameObjects() 
  {
    ...
    //let slimeNum = this.slimeNum; //initialize our slime numbering used to check if the slime has been killed
    let regName
  ...
  createSlime() {
    ...
    const scene = this;
    let slimeNum = this.slimeNum;
    let regName = `${level}_Slime_${slimeNum}`;
    let slime = new Slime({
        scene: scene,
        x: player.x + 32, 
        y: player.y + 32,
